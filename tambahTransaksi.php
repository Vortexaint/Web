...
<head>
    ...
    <title>Tambah Transaksi - Vortex Mart</title>
    <style>
        /* Basic Reset */
        * {
            margin: 0;
...
<div class="container">
        ...
        <form action="" method="post">
            <div class="form-group">
                ...
                <select name="idBarang" required>
                    ...
                    while ($d = mysqli_fetch_assoc($data)) {
                        echo "<option value='{$d['idBarang']}'>{$d['idBarang']} ({$d['namaBarang']})</option>";
                    }
                    ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="idKasir" required>
                    ...
                    while ($d = mysqli_fetch_assoc($data)) {
                        echo "<option value='{$d['idKasir']}'>{$d['idKasir']} ({$d['namaKasir']})</option>";
                    }
                    ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="idPelanggan" required>
                    ...
                    while ($d = mysqli_fetch_assoc($data)) {
                        echo "<option value='{$d['idPelanggan']}'>{$d['idPelanggan']} ({$d['namaPelanggan']})</option>";
                    }
                    ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <input type="number" name="jumlahBarang" required>
            </div>
            <div class="form-group">
                <label>Tanggal:</label>
                <input type="date" name="tanggalTransaksi" required>
            </div>
        ...
        if (isset($_POST['submit'])) {
            ...
            // Insert new data
            $query = "INSERT INTO transaksi (idBarang, idKasir, idPelanggan, jumlahBarang, tanggalTransaksi) VALUES ('$idBarang', '$idKasir', '$idPelanggan', '$jumlahBarang', '$tanggalTransaksi')";
