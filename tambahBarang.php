...
<head>
    ...
    <title>Tambah Barang - Vortex Mart</title>
    <style>
        /* Basic Reset */
        * {
            margin: 0;
...
<body>
    ...
    <div class="container">
        ...
        <form action="" method="post">
            ...
            <div class="form-group">
                <label>Stok:</label>
                <input type="number" name="stokBarang" required>
            </div>
            <div class="form-group">
                <label>Jenis:</label>
                <select name="idJenis" required>
                    ...
                    while ($d = mysqli_fetch_assoc($data)) {
                        echo "<option value='{$d['idJenis']}'>{$d['idJenis']} ({$d['namaJenis']})</option>";
                    }
                    ?>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="idRak" required>
                    ...
                    while ($d = mysqli_fetch_assoc($data)) {
                        echo "<option value='{$d['idRak']}'>{$d['idRak']} ({$d['namaRak']})</option>";
                    }
                    ?>
                </select>
            </div>
        ...
        if (isset($_POST['submit'])) {
            ...
            // Reset AUTO_INCREMENT if needed (use correct primary key column name here, e.g., idBarang)
            $result = mysqli_query($koneksi, "SELECT MAX(idBarang) AS max_id FROM barang");
            $row = mysqli_fetch_assoc($result);
            $maxId = $row['max_id'];
            $nextId = $maxId + 1;

            mysqli_query($koneksi, "ALTER TABLE barang AUTO_INCREMENT = $nextId");

            // Insert new data
            $query = "INSERT INTO barang (namaBarang, stokBarang, idJenis, hargaBarang, idRak) VALUES ('$namaBarang', '$stokBarang', '$idJenis', '$hargaBarang', '$idRak')";
    ...
    <script>
        ...
        // Add event listener to format input as currency
        document.getElementById("hargaBarang").addEventListener("input", function (e) {
            let cleanedValue = e.target.value.replace(/[^,\d]/g, "");
            e.target.value = formatRupiah(cleanedValue);
        });
    </script>
</body>
</html>
